...
{
  LPC_RIT->RICTRL |= (1<<3);	// Start timer counter
  return;
}
...
{
  LPC_SC->PCLKSEL1  &= ~(3<<26);   // All 1s except positions 26 and 27 (we clear RIT bits)
  LPC_SC->PCLKSEL1  |=  (1<<26);   // RIT Clock = CCLK (1 MHz clock)
